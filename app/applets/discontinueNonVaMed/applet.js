...
function onResolveDependencies(ADK, DiscontinueTemplate, DiscontinueFooterTemplate, DiscontinueReasonTemplate) {
    ...
    var rootItemView = Backbone.Marionette.LayoutView.extend({
        ...
        'showModal': function(event, options) {
            ...
            var titleView = Backbone.Marionette.ItemView.extend({
                template: _.template("<button type='button' class='close' data-dismiss='modal'><span aria-hidden='true'>×</span><span class='sr-only'>Close</span></button><h4 class='modal-title'><span class='text-danger'>Discontinue</span> <%= name %></h4>")
            });
