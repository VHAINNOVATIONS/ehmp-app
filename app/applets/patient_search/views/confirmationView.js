...
function onResolveDependencies(Backbone, Marionette, _, confirmationTemplate, acknowledgeTemplate, patientFlagTemplate, blankTemplate, ADK, SessionStorage, CCOWService, ScreenBuilder, UserDefinedScreens) {
    ...
    var ConfirmationView = Backbone.Marionette.ItemView.extend({
        ...
        maskSSN: function(patientModel) {
            var maskedSSN = patientModel.get('ssn').replace(/(?=\d{5})\d/gmi, '*');
        ...
        confirmPatient: function(event) {
            ...
            if ("ActiveXObject" in window && CCOWService.getCcowStatus() === 'Connected') {
                CCOWService.setContext(patient);
                this.updateTemplateToBlank();
            } else {
                UserDefinedScreens.screensConfigNullCheck();
                var initRoutersPromise = ScreenBuilder.initAllRouters(ADK.ADKApp);
                initRoutersPromise.done(function() {
                    ScreenBuilder.buildAll(ADK.ADKApp);
