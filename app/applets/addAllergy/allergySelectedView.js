...
function onResolveDependencies(Backbone, Marionette, _, Moment, modalTemplate, footerTemplate, SymptomsView, SymptomsSelectedView, SymptomsScrollEventHandler, SymptomsSearchEventHandler, AllergiesUtil, SymptomsUtil, ModelBindingUtil, AllergenModel, ADK) {
    'use strict';
    ...
    var AllergySelectedView = Backbone.Marionette.LayoutView.extend(ModelBindingUtil).extend({
        ...
        showModal: function(destroy) {
            ...
            var titleView = Backbone.Marionette.ItemView.extend({
                template: _.template("<button type='button' class='close' data-dismiss='modal'><span aria-hidden='true'>×</span><span class='sr-only'>Close</span></button><h4 class='modal-title'>Enter Allergy/Adverse Reaction for <b> <%= name %> </b></h4>")
            });
