var dependencies = [
    ...
    "main/ADK",
    "ADKApp",
    "hbs!app/applets/medication_review/list/medicationTemplate",
    "app/applets/medication_review/list/orderDetailView",
    "hbs!app/applets/medication_review/list/orderHistoryTemplate",
    "hbs!app/applets/medication_review/list/infoButtonTemplate",
    "hbs!app/applets/medication_review/list/infoButtonTemplate2",
    "hbs!app/applets/medication_review/list/infoButtonPatientTemplate",
    "app/applets/medication_review/appletHelper"
];

define(dependencies, onResolveDependencies);

function onResolveDependencies(Backbone, Marionette, ADK, adkApp, medicationTemplate, OrderDetailView, orderHistoryTemplate, infoButtonTemplate, infoButtonTemplate2, infoButtonPatientTemplate, appletHelper) {
    ...
    var MedicationItemView = Backbone.Marionette.LayoutView.extend({
        initialize: function() {
            ...
            this.orderHistoryListView = new orderHistoryView({
                collection: this.model.get("meds"),
                parentLayoutView: parent
            });

            this.orderDetailView = new OrderDetailView({
                model: this.model.get("meds").models[0]
            });
            ...
            this.model.set('activeCount', this.countMedStatus(this.model.get('meds'), 'ACTIVE'));
            this.model.set('pendingCount', this.countMedStatus(this.model.get('meds'), 'PENDING'));
        ...
        templateHelpers: {
            isNonVa: function() {
                if (parent.model.get("meds").models[0].get("vaType") === 'N') {
                    return true;
                } else {
                    return false;
                }
        ...
        countMedStatus: function(model, status) {
            var count = 0;
            model.forEach(function(item) {
                if (item.get('standardizedVaStatus').toUpperCase() === status) {
                    count++;
                }
            });
            if (count > 1) {
