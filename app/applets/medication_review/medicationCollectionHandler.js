...
function onResolveDependencies(ADK, appletHelper) {
    ...
    var collectionHandler = {
        ...
        fetchAllMeds: function() {
            ...
            if (dateModel.get('fromDate') && (dateModel.get('fromDate') !== 'null')) {
                ...
                var overallStop = ADK.utils.formatDate(dateModel.get('toDate'), 'YYYYMMDD', 'MM/DD/YYYY');
                fetchOptions.criteria.filter = 'and(gte(overallStart,"' + overallStart + '"),lte(overallStop,"' + overallStop + '"))';
            }

            this.allMedications = ADK.PatientRecordService.fetchCollection(fetchOptions);

            orderValue = function(vaStatus) {
