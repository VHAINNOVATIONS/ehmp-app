...
function onResolveDependencies(Backbone, Marionette, $, _, BootstrapModalTemplate, BootstrapModalHeaderTemplate) {
        ...
        generateDiv: function(modalOptions, ModalLayoutView) {
            var regionName = modalOptions.regionName || 'mainModalDialog',
                temp = Backbone.Marionette.LayoutView.extend({
                    ...
                    template: _.template('<div id="<%= regionName %>" class="modal fade" role="dialog" tabindex="-1" data-backdrop="<%= backdrop %>" data-keyboard="<%= keyboard %>" aria-hidden="true"></div>'),
                    tagName: 'div',
                    attributes: {
                        'id': 'mainModal',
                    },
