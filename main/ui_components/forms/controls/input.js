define([
    ...
    var Input = PuppetForm.InputControl = PuppetForm.DefaultInputControl.extend({
        getTemplate: function() {
            ...
            var characterCount = '{{#if maxlength}}{{#if charCount}}<span><span class="input-char-count left-margin-xs"></span> characters remaining</span>{{/if}}{{/if}}';
            ...
            var unitsAsArray_2_Elements = Handlebars.compile([
                ...
                '</div>',
                '{{#if units}}',
                '<div class="col-xs-5 radio radio-and-input right-padding-no top-margin-md bottom-margin-xs">',
                '{{#each units}}',
