define([
    ...
    var NoChildrenView = Backbone.Marionette.ItemView.extend({
      ...
      selectedTemplate: Handlebars.compile('<p>No {{keyword}} found</p>{{#each selectedItems}}<div/>{{/each}}<div class="pixel-width-35"/>'),
    ...
    var SelectedChildView = Backbone.Marionette.CompositeView.extend({
      ...
      buttonTemplate: Handlebars.compile('<div><button type="button" class="btn btn-icon all-padding-no pull-right checked-{{value}}" title="Press enter to remove {{label}}."><i class="right-padding-sm left-padding-sm fa fa-times-circle"></i></button></div>'),
            ...
            return {
              ...
              view: function() {
                var containerField = new PuppetForm.Field({
                  control: 'container',
                  items: [model.toJSON()],
                  extraClasses: [model.get('extraClasses')] || []
                });
      ...
      onRender: function() {
        this.$el.append(this.buttonTemplate(this.serializeData()));
      }
    });
    var AvailableChildView = Backbone.Marionette.ItemView.extend({
        ...
        template: Handlebars.compile([
            '<div class="row">',
                ...
                '<div class="col-xs-2">',
                    '<button type="button" class="btn btn-icon all-padding-no checked-{{value}}" title="Press enter to {{#if value}}remove{{else}}add{{/if}} {{label}}."><i class="right-padding-xs left-padding-xs fa {{#if value}}fa-times-circle{{else}}fa-plus{{/if}}"></i></button>',
                '</div>',
            '</div>'
        ].join("\n")),
        modelEvents: {
    ...
    var MultiselectSideBySideCompositePrototype = {
        ...
        selectedTemplate: Handlebars.compile([
          '<div class="faux-table">',
            '<div class="sr-only"> This table contains the available options to select</div>',
            '<div class="header">',
              '<div class="table-row">',
                  '<div>Selected</div>',
                '{{#each selectedItems}}'+
                  '<div class="{{this.extraClasses}}">{{this.headerName}}</div>'+
                '{{/each}}',
                  '<div class="pixel-width-35"> </div>',
              '</div>',
            '</div>',
        ...
        availableTemplate: Handlebars.compile([
            '<div class="panel panel-default panel-w-list-group">',
                ...
                '<div class="panel-heading panel-heading-small">',
                    ...
                    '</div>',
                '</div>',
                '<div class="row left-padding-xs right-padding-xs">',
    ...
    var MultiselectSideBySidePrototype = {
        ...
        initialize: function(options) {
            ...
            var self = this;
            this.listenTo(this.collection, 'change:'+self.attributeMapping.value, function() {
