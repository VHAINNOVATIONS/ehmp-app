...
*
* Modified: 2014, Jan 16 to config corners on
* button, checkboxradio, collapsible, collapsible-set,
* slider/toggle, textinput; config shadow respected on button (alisa.ky.chen)
*
* Modified: 2014, Jan 26 to add class .ui-btn-inner-large for large LV icons and
* add class .ui-icon-lv for LV icons and .ui-icon-lv-large for large LV icons (alisa.ky.chen)
*
* Modified: 2014, Mar 27 to set focus on clicked anchors styled with
* .ui-btn or .ui-btn-inherit; set focus back to previously active element when closing popups
* (alisa.ky.chen)
*
* Modified: 2014, Mar 31 bugfix, (introduced by hiding off-screen popups)
* allow custom selectmenu to calculate height in _decideFormat (alisa.ky.chen)
*
* Modified: 2014, Apr 01 to switch blues in theme b;
* navbars able to take theme option (alisa.ky.chen)
*
* Modified: 2014, Apr 10 correct calculation of height for selectmenu popup
* to not include h1 or tolerances (alisa.ky.chen)
*/
...
(function( $, window, undefined ) {
	...
	$.mobile = $.extend($.mobile, {
		...
		getInheritedTheme: function( el, defaultTheme ) {
			...
			while ( e ) {
				c = e.className || "";
				if ( c && ( ( m = re.exec( c ) ) && ( ltr = m[ 2 ] ) ) || ( e.getAttribute && e.getAttribute('data-content-theme') && ( ltr = e.getAttribute('data-content-theme') ) ) ) {
...
$.fn.buttonMarkup = function( options ) {
	var $workingSet = this,
		...
		key,
		patternLVIcon = new RegExp('^lv-'),
		patternLVLargeIcon = new RegExp('^lv-large-');
	...
	for ( var i = 0; i < $workingSet.length; i++ ) {
		var el = $workingSet.eq( i ),
			...
			o = $.extend( {}, $.fn.buttonMarkup.defaults, {
				...
				theme:      options.theme      !== undefined ? options.theme      : getAttrFixed( e, nsKey + "theme" ) || $.mobile.getInheritedTheme( el, "b" ),
			...
			// Classes Defined
			isLVIcon = o.icon !== undefined && patternLVIcon.test(o.icon),
			isLVLargeIcon = o.icon !== undefined && patternLVLargeIcon.test(o.icon),
			innerClass = "ui-btn-inner" + ( ("lv-provider-menu" === o.icon || "lv-folder-flip" === o.icon || "lv-large-provider-menu" === o.icon || "lv-large-folder-flip" === o.icon) ? " ui-btn-inner-lv-extend" : "" ),
		...
		if ( !o.theme ) {
			o.theme = $.mobile.getInheritedTheme( el, "b" );
		}

		buttonClass = "ui-btn ";
		buttonClass += (isLVIcon ? ( isLVLargeIcon ? "ui-btn-lv-large " : "ui-btn-lv " ) : "" );
		...
		if ( o.icon ) {
			...
			iconClass = "ui-icon " + (isLVIcon ? ( isLVLargeIcon ? "ui-icon-lv-large " : "ui-icon-lv " ) : "" ) + o.icon;
...
$.mobile.document.bind( "pagecreate create", function( e ) {
	...
	$( ":jqmData(role='button'), .ui-bar > a, .ui-header > a, .ui-footer > a, .ui-bar > :jqmData(role='controlgroup') > a", e.target )
		...
		.each(function(i, el) {
			// corners are round on popup close buttons by default
			corners = $(el).jqmData('corners');
			shadow = $(el).jqmData('shadow');
			iconshadow = $(el).jqmData('iconshadow');
			parent = $(el).parent();
			isPopupCloseBtn = parent.jqmData('role') === 'popup' || parent.is('.ui-popup');

			$(el).buttonMarkup({
				corners: (typeof corners !== 'undefined') ? corners : ( isPopupCloseBtn ? true : $.mobile.button.prototype.options.corners),
				shadow: (typeof shadow !== 'undefined') ? shadow : $.mobile.button.prototype.options.shadow,
				iconshadow: (typeof iconshadow !== 'undefined') ? iconshadow : $.mobile.button.prototype.options.iconshadow
			});
		});
});
...
$.widget( "mobile.collapsible", $.mobile.widget, {
	options: {
		...
		expandText: " contents have been expanded",
		collapseText: " contents have been collapsed",
		collapsed: true,
	...
	_create: function() {
		...
		if ( collapsibleSet.length ) {
			...
			if ( !o.theme ) {
				o.theme = collapsibleSet.jqmData( "theme" ) || $.mobile.getInheritedTheme( collapsibleSet, "b" );
			}
		...
		collapsible
			.bind( "expand collapse", function( event ) {
				if ( !event.isDefaultPrevented() ) {
					...
					$target = $(event.currentTarget).find('[aria-controls]')
					var liveId = $target.attr('aria-controls');
					if (liveId) {
							var $liveDiv = $('#' + liveId);
							var title = $liveDiv.attr('data-title');
						if (title === undefined) {
							title = '';
						}
						var newText = title + (isCollapse ? o.collapseText : o.expandText);
							$('#live-regions').attr('aria-busy','true');
							$('.live-item').text('');
							$liveDiv.text(newText);
							$('#live-regions').removeAttr('aria-busy');
						}
					}
...
$.widget( "mobile.navbar", $.mobile.widget, {
	...
	_create: function() {
		...
		var $navbar = this.element,
			...
			theme = $navbar.jqmData('theme') || $.mobile.getInheritedTheme( $navbar, "b" ),
...
$.widget( "mobile.listview", $.mobile.widget, $.extend( {
	...
	refresh: function( create ) {
		...
		if ( !o.theme ) {
			o.theme = $.mobile.getInheritedTheme( this.element, "b" );
		}
...
$.widget( "mobile.textinput", $.mobile.widget, {
	...
	_create: function() {
		...
		var self = this,
			...
			cornerClass = !!o.corners ? (isSearch ? "ui-btn-corner-all" : " ui-corner-all") : "",
			theme = o.theme || $.mobile.getInheritedTheme( this.element, "b" ),
			...
			clearBtnCorners = o.clearBtnCorners,
			clearBtnShadow = o.clearBtnShadow,
		...
		if ( isSearch ) {
			focusedEl = input.wrap( "<div class='ui-input-search ui-shadow-inset ui-btn-shadow ui-icon-searchfield" + cornerClass + themeclass + miniclass + "'></div>" ).parent();
		} else if ( inputNeedsWrap ) {
			focusedEl = input.wrap( "<div class='ui-input-text ui-shadow-inset ui-btn-shadow" + cornerClass + themeclass + miniclass + "'></div>" ).parent();
		}
		...
		if( inputNeedsClearBtn || isSearch ) {
			clearbtn = $( "<a href='#' class='ui-input-clear' title='" + clearBtnText + "'>" + clearBtnText + "</a>" )
				...
				.buttonMarkup({
					icon: "delete",
					iconpos: "notext",
					corners: clearBtnCorners,
					shadow: clearBtnShadow,
					mini: o.mini
				});
			...
			input.addClass( "ui-shadow-inset" + cornerClass + themeclass + miniclass );
		}
...
$.widget( "mobile.button", $.mobile.widget, {
	...
	_create: function() {
		...
		if ( !this.options.theme ) {
			this.options.theme = $.mobile.getInheritedTheme( this.element, "b" );
		}
...
$.widget( "mobile.slider", $.mobile.widget, $.extend( {
	...
	_create: function() {
		...
		var self = this,
			...
			parentTheme = $.mobile.getInheritedTheme( control, "b" ),
			theme = this.options.theme || parentTheme,
			trackTheme = this.options.trackTheme || parentTheme,
			handleCorners = !this.isToggleSwitch || this.options.handleCorners,
			handleShadow = this.options.handleShadow,
			...
			trackCornersClass = this.options.trackCorners ? " ui-btn-corner-all" : "",
			...
			valuebg = this.options.highlight && !this.isToggleSwitch ? (function() {
				...
				bg.className = "ui-slider-bg " + $.mobile.activeBtnClass + trackCornersClass;
		...
		domSlider.className = [this.isToggleSwitch ? "ui-slider " : "ui-slider-track ",selectClass," ui-btn-down-",trackTheme, trackCornersClass, miniClass].join( "" );
		domHandle.className = "ui-slider-handle";
		domSlider.appendChild( domHandle );

		handle.buttonMarkup({ corners: handleCorners, theme: theme, shadow: handleShadow })
				.attr({
					"role": "slider",
					"aria-valuemin": min,
					"aria-valuemax": max,
		...
		if ( this.isToggleSwitch ) {
			...
			for ( var i = 0, optionsCount = options.length; i < optionsCount; i++ ) {
				var side = !i ? "b" : "a",
					...
					sliderCornersClass = this.options.sliderCorners ? " ui-btn-corner-all" : "",
					sliderLabel = document.createElement( "div" ),
					sliderImg = document.createElement( "span" );

				sliderImg.className = ["ui-slider-label ui-slider-label-", side, sliderTheme, sliderCornersClass].join( "" );
	...
	refresh: function( val, isfromControl, preventInputUpdate ) {
		...
		var self = this,
			parentTheme = $.mobile.getInheritedTheme( this.element, "b" ),
			theme = this.options.theme || parentTheme,
			trackTheme = this.options.trackTheme || parentTheme,
			handleCorners = !this.isToggleSwitch || this.options.handleCorners,
			handleShadow = this.options.handleShadow,
		...
		self.slider[0].className = [ this.isToggleSwitch ? "ui-slider ui-slider-switch" + sliderCornersClass : "ui-slider-track" + trackCornersClass," ui-btn-down-" + trackTheme, ( this.options.mini ) ? " ui-mini":""].join( "" );
		...
		this.handle.buttonMarkup({ corners: handleCorners, theme: theme, shadow: handleShadow });
...
$.widget( "mobile.selectmenu", $.mobile.widget, $.extend( {
	...
	_preExtension: function() {
		...
		if ( !this.options.theme ) {
			this.options.theme = $.mobile.getInheritedTheme( this.select, "b" );
		}
	},
	...
	$.widget( "mobile.popup", $.mobile.widget, {
			...
			if ( this._isOpen ) {
				if ( !this._expectResizeEvent() ) {
					if ( this._ui.container.hasClass( "ui-popup-hidden" ) ) {
						...
						this.reposition( { positionTo: this.options.positionTo || "window" } );
						this._trigger("afterwindowresize");
		...
		_create: function() {
			var ui = {
					...
					container: $( "<div class='ui-popup-container ui-popup-hidden' role='complementary'></div>" )
				},
		...
		_open: function( options ) {
			...
			if ( !this.options.theme ) {
				this._setTheme( this._page.jqmData( "theme" ) || $.mobile.getInheritedTheme( this._page, "b" ) );
			}
		...
		_closePrereqsDone: function() {
			...
			$(this._focusedElementBeforeOpen).filter(":visible").focus();
		...
		$.extend( widget, {
			...
			_decideFormat: function() {
				var self = this,
					...
					selfListContainer = self.list.parent().parent().css("display", "block"),
				...
				if ( menuHeight > screenHeight - 4 || !$.support.scrollTop ) {
...
(function( $, undefined ) {
	// Set focus on click for any enhanced link (to match behaviour of inputs, buttons, submits);
	// only click fires in time before popups open
	$( document ).on("click", "a.ui-btn, a.ui-link-inherit", function(e) {
		$(e.currentTarget).filter(":visible").focus();
	});
...
$.widget( "mobile.panel", $.mobile.widget, {
	...
	_create: function() {
		var self = this,
			$el = self.element,
			page = $el.closest( ":jqmData(role='page'), :jqmData(role='dialog'), [data-role=page], [data-role=dialog]").first(),
		...
		$.extend( this, {
			...
			_page: $el.closest( ":jqmData(role='page'), :jqmData(role='dialog'), [data-role=page], [data-role=dialog]").first(),
		...
		if ( !this._open ) {
			var self = this,
				...
				complete = function() {
					...
					var selectableElements = $(this).find("a[href], link[href], button, input, select, textarea, div[draggable]").not("[style='display:none']");
